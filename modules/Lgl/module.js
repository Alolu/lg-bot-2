...
export default class Lgl extends Module {
    constructor() {
        ...
        this.lgJoin = new this.Command(
            ...
            game = this.gameStatus.checkIfGameExists(this.gamesList, arg[0]);
            if (!game) {
        ...
        this.lgUpdate = new this.Command('<"name" or "playernumber"><new name or number of players>', 'Change the parameters of the game', [{ type: 'string' }, { type: 'string' },
        { type: 'string', optional: "true" }, { type: 'string' }], function (msg, arg) {
            ...
            partyPanel = this.gameStatus.showPartyPanel(new Discord.RichEmbed(), this.format, this.gamesList.get(msg.author));
            msg.reply(partyPanel);
        });
    }
}
